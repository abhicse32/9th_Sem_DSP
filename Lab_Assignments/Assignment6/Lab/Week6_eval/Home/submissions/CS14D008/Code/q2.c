...
void printfixedexp(Node* T)
{
	if(isalnum(T->data[0])||strlen(T->data)>1)
		printf("%s ",T->data);
	else
	{
		if(precedence(T->left->data[0])<=precedence(T->data[0]))
		{
			printf("( ");
			printfixedexp(T->left);
			printf(") ");
		}
		else
			printfixedexp(T->left);
		...
		if(precedence(T->right->data[0])<=precedence(T->data[0]))
		{
			printf("( ");
			printfixedexp(T->right);
			printf(") ");
		}
		else
			printfixedexp(T->right);
