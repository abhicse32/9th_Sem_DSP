...
void pushStack ( int which , char what , int option )
{
   if (option == 0) {
      if (which == 1) {
         tos1 += 2;
         if (tos1 >= MAX_SIZE) {
            fprintf(stdout,"Error: Overflow in Stack 1.\n");
            exit(1);
         } else store[tos1] = what;
      } else if (which == 2) {
         tos2 += 2;
         if (tos2 >= MAX_SIZE) {
            fprintf(stdout,"Error: Overflow in Stack 2.\n");
            exit(1);
         } else store[tos2] = what;
      } else {
         fprintf(stdout,"Error: Undefined stack identifier.\n");
         exit(1);
      }
   } else {
      if (which == 1) {
         ++tos1;
         if (tos1 >= tos2) {
            fprintf(stdout,"Error: Overflow in stack.\n");
            exit(1);
         } else store[tos1] = what;
      } else if (which == 2) {
         --tos2;
...
void popStack ( int which , int option )
{
   if (option == 0) {
      if (which == 1) {
         if (tos1 < 0) {
            fprintf(stdout,"Error: Underflow in Stack 1.\n");
            exit(1);
         } else {
            store[tos1] = '_';
            tos1 -= 2;
         ...
         if (tos2 < 0) {
            fprintf(stdout,"Error: Underflow in Stack 2.\n");
            exit(1);
         } else {
            store[tos2] = '_';
            tos2 -= 2;
