...
void pushStack ( int which , char what , int option )
{
   if (option == 0) { //for odd-even strategy
	...
	if(which ==1) 
	{
		if(tos1 == MAX_SIZE) {printf("Overflow­Error\n");exit(1);}
		else{store[tos1] = what; tos1 =tos1+2;}
	}
	else 
	{
		if(tos2 == MAX_SIZE) {printf("Overflow­Error\n");exit(1);}
		else{store[tos2] = what; tos2 =tos2+2;}
   	}
}   else { // for colliding strategy
      // Write C-code Here
	if(which ==1)
	...
	{
		if(tos2 == 1) {printf("Overflow­Error\n");exit(1);}
		else{store[tos2] = what; tos2 =tos2-2;}
   	}
   }
}
void popStack ( int which , int option )
{
   if (option == 0) { //for odd-even strategy
	...
	if(which ==1)	
	{
		if(tos1 == 0) {printf("Underflow­Error");exit(1);}
		else {store[tos1-2] = '_'; tos1 =tos1-2;}
	}
	else	
	{
		if(tos2 == 0) {printf("Underflow­Error");exit(1);}
		else {store[tos2-2] = '_'; tos2 =tos2-2;}
   	}
}
   else { // for colliding strategy
      // Write C-code Here
	...
	{
		if(tos2 == MAX_SIZE-1){printf("Underflow­Error");exit(1);}
		else{store[tos2+2] = '_'; tos2 =tos2+2;}
   	}
}
}
