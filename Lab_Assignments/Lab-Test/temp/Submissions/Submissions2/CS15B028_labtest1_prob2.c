...
void pushStack ( int which , char what , int option )
{
   if (option == 0) { //for odd-even strategy
      ...
      if(which==1)
      {
        if(tos1>=30) {printf("Error: Overflow in Stack 1.\n"); exit(0);}
        else
        {
           tos1=tos1+2;
           store[tos1]=what;
      ...
      if(which==1)
      {
        if(tos1+1==tos2) {printf("Error: Overflow in Stack 1.\n"); exit(0);}
        else 
        {
           tos1++;
           store[tos1]=what;
      ...
      {
        if(tos2-1==tos1) {printf("Error: Overflow in Stack 2.\n"); exit(0);}
        else
        {
           tos2--;
           store[tos2]=what;
...
void popStack ( int which , int option )
{
   if (option == 0) { //for odd-even strategy
      ...
      {
        if(tos2==32) {printf("Error: Underflow in Stack 2.\n"); exit(0);}
