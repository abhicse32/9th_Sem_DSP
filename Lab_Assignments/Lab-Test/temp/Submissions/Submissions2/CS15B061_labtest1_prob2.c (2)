...
void pushStack ( int which , char what , int option )
{
   if (option == 0) { //for odd-even strategy
      // Write C-code Here
	   if(which==1){
	   	if(tos1+2>=MAX_SIZE) {printf("Error: Overflow in Stack 1\n"); exit(0);}
		store[tos1+=2]=what;
	   }
	   else if(which==2){
	   	if(tos2+2>=MAX_SIZE) {printf("Error: Overflow in Stack 2\n"); exit(0);}
		store[tos2+=2]=what;
	   }
   }
   else { // for colliding strategy
      // Write C-code Here
	   if(tos2-tos1<=1) {printf("Error: Overflow in Stack\n"); exit(0);}
	   if(which==1) store[++tos1]=what;
	   else if(which==2) store[--tos2]=what;
   }
}
...
void popStack ( int which , int option )
{
   if (option == 0) { //for odd-even strategy
      ...
      // Write C-code Here
	   if(which==1){
	   	if(tos1==-1) {printf("Error: Underflow in Stack 1\n"); exit(0);}
		store[tos1--]='_';
	   }
	   else if(which==2){
	   	if(tos2==MAX_SIZE) {printf("Error: Underflow in Stack 2\n"); exit(0);}
		store[tos2++]='_';
	   }
   }
}
